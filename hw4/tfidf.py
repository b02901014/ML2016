...
km = KMeans(n_clusters=n_cluster[2], init='k-means++', max_iter=100, n_init=10)
km.fit(center2)
label3 = np.asarray(km.labels_)
center3 = km.cluster_centers_
clusterSize3 = np.zeros((n_cluster[2],1))
for i in range(len(label3)):
    clusterSize3[label3[i]]+=clusterSize2[i]
    index2 = ((label2==i)*1).nonzero()
    finalLabel[index2] = label3[i]
...
with open(file_dir, 'wb') as ans:
    ...
    for j in range(myData.shape[0]):
        ...
        a = myData[j][1]
        b = myData[j][2]
        if finalLabel[a,0] == finalLabel[b,0]:
            result = 1
